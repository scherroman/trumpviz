$.getScript("/static/scripts/helper_methods.js", function(){
  ...
  attributes = ["is_retweet","u_followers","u_friends","u_statuses","u_favorites","rt_u_followers","rt_u_statuses"];
  ...
  var positive_colors = d3.scaleLinear().range(["white", "red"]);
  var negative_colors = d3.scaleLinear().range(["white", "blue"]);

  d3.csv("/static/500_tweets_sample_trump_formatted_augmented_correlation_matrix_numeric_only.csv", correlation_matrix);
  // d3.csv("https://gist.githubusercontent.com/emeeks/6decab485fd2e02c9f1e/raw/9e4d0d1c427d565aa7e3099120f0eca342d035a2/diamonds.csv", correlation_matrix);
  ...
  function correlation_matrix(data) {
    ...
    x_axis = g.append("g")
          ...
          .call(d3.axisTop(x).ticks(X_AXIS_TICKS))

    //Rotate x-axis, center labels
    x_axis.selectAll("text")
          .style('text-anchor', 'end')
          .attr('transform', 'translate(-25)rotate(90 0 -10)')
    ...
    .each(function (matrix, i) {
      ...
      row = data[Math.floor(i / 7)]
      current_attribute = attributes[i % 7]
      ...
      if (isNaN(corr_coef)) {
        ...
        chosen_color = positive_colors(corr_coef)
      }
      else if (corr_coef < 0) {
        chosen_color = negative_colors(Math.abs(corr_coef))
      }
      ...
      //background/border
      d3.select(this).append("rect").style("fill", chosen_color).style("stroke", "black").style("stroke-width", 1)
      .attr("height", 50)
      .attr("width", 50);
